...
import com.example.AIForYouAPI.dtos.NeuralParams;
import com.example.AIForYouAPI.tools.DenseLayer;
import com.example.AIForYouAPI.tools.Matrix;
...
public class SimpleNeuralNetwork {
    ...
    public NeuralParams getParams(Pair<Vector, Float>[] test) throws Exception {
        Vector[][] wParams = new Vector[layers.length][];
        Vector[] bParams = new Vector[layers.length];

        for(int i=0;i<wParams.length;i++) {
            wParams[i] = layers[i].getW();
            bParams[i] = layers[i].getB();
        }

        return new NeuralParams(wParams, bParams, errorOnData2(test));
    }

    private double errorOnData(Pair<Vector, Vector>[] set) throws Exception {
        double total = 0;

        for(Pair<Vector, Vector> p : set) {
    ...
    private double errorOnData2(Pair<Vector, Float>[] set) throws Exception {
        double total = 0;
